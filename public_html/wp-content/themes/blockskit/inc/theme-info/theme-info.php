...
if ( ! class_exists( 'Bk_Theme_Info' ) ) {
    class Bk_Theme_Info{
        ...
        public function prepare_class() {
            ...
            $this->page_title    = $this->theme_name . esc_html__( ' Info', 'blockskit' );
            $this->menu_title    = $this->theme_name . esc_html__( ' Theme', 'blockskit' );
                    ...
                    foreach ( $gs_steps as $gs_step ) {
                        ...
                        if( $is_gs ) {
                            if ( is_plugin_active( 'blockskit/blockskit.php' ) && is_plugin_active( 'advanced-import/advanced-import.php' ) ) {
                                echo ('<h3>'.esc_html__( 'Getting Started', 'blockskit' ).'</h3>');
                                echo ('<p>'.esc_html__( 'Essential plugins are added successfully.', 'blockskit' ).'</p>');
                            echo '</div>';
                            }else {
                                if ( ! empty( $title ) ) {
                                    echo '<h3>';
        ...
        public function recommended_actions() {
            ...
            if ( ! empty( $recommended_actions ) ) {
                        ...
                        if ( ! empty( $plugin_slug ) ) {
                            ...
                            switch ( $active['needs'] ) {
                                ...
                                case 'install':
                                    ...
                                    $label = esc_html__( 'Install', 'blockskit' );
                                    break;

                                case 'activate':
                                    $class = 'activate-now button button-primary';
                                    $label = esc_html__( 'Activate', 'blockskit' );

                                    break;

                                case 'deactivate':
                                    $class = 'deactivate-now button';
                                    $label = esc_html__( 'Deactivate', 'blockskit' );
                ...
                if ( $hooray ){
                    echo '<span class="hooray">' . esc_html__( 'Hooray! There are no recommended actions for you right now.', 'blockskit' ) . '</span>';
                    echo '<a data-action="reset" id="reset" class="reset-all" href="#">'.esc_html__('Show All Recommended Actions', 'blockskit').'</a>';
                }
                echo '</div>';
            }
        }
                        ...
                        if( !empty( $screenshot) ){
                            ...
                            if ( ! empty( $title ) ) {
                                ...
                                if ( ! empty( $download_link ) ) {
                                    echo '<a href="' . esc_url( $download_link ) . '" class="button button-primary install right">' . esc_html__( 'Download', 'blockskit' ) . '</a>';
                                }
                                if ( ! empty( $preview_link ) ) {
                                    echo '<a class="button button-secondary preview right" target="_blank" href="' . $preview_link . '">' . esc_html__( 'Live Preview', 'blockskit' ). '</a>';
                                }
                                echo '</div>';
                                echo '</div>';
                            }
        ...
        public function free_pro() {
            ...
            if ( ! empty( $free_pro ) ) {
                ...
                if ( ! empty( $free_pro ) && is_array( $free_pro ) ) {
                    ...
                    echo '<thead>';
                    echo '<tr>';
                    echo '<th>' . esc_html__( 'Theme Features','blockskit' ) . '</th>';
                    echo '<th>' . esc_html__( 'Free','blockskit' ) . '</th>';
                    echo '<th>' . esc_html__( 'Pro (Premium)','blockskit' ) . '</th>';
                    echo '</tr>';
                    echo '</thead>';
                    echo '<tbody>';
...
$config = array(
    ...
    // Main welcome title
    'info_title' => sprintf( esc_html__( 'Welcome to %s - ', 'blockskit' ), $theme_name_config ),

    // Main welcome content
    'info_content' => sprintf( esc_html__( '%s is now installed and ready to use. We hope the following information will help and you enjoy using it!', 'blockskit' ), '<b>'.$theme_name_config.'</b>' ),
    ...
    'quick_links' => array(
        'theme_url'  => array(
            'text' => __( 'Theme Info', 'blockskit' ),
            'url' => 'https://blockskit.com/' .$theme_name_config_url 
        ),
        'pro_url'  => array(
            'text' => __( 'Buy Pro', 'blockskit' ),
            'url' => 'https://blockskit.com/pricing'
        ),
    ),

    'tabs' => array(
        'getting_started'      => esc_html__( 'Getting Started', 'blockskit' ),
        'free_pro'             => esc_html__( 'Compare Free Vs Pro', 'blockskit' ),
        'recommended_plugins'  => esc_html__( 'Recommended Plugins', 'blockskit' ),
        'changelog'            => esc_html__( 'Changelog', 'blockskit' ),
    ),

    /*Getting started tab*/
    'gs_steps' => array(
        'one' => array(
            'title' => esc_html__( 'Get Started', 'blockskit' ),
            'desc' => esc_html__( 'By clicking "Get Started," you will install and activate essential plugins to prepare your demo import.', 'blockskit' ),
            'button_label' => sprintf(esc_html__( 'Get started with %s', 'blockskit' ), $theme_name_config ),
        ...
        'two' => array(
            'title' => esc_html__( 'Pre-built Demos', 'blockskit' ),
            'desc' => esc_html__( 'We recommend utilizing our collection of pre-designed starter site templates as an efficient and convenient way to begin your project.', 'blockskit' ),
            'button_label' => esc_html__( 'Browse Demos', 'blockskit' ),
            'button_link' => esc_url( 'https://blockskit.com/starter-sites/' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'three' => array (
            'title' => esc_html__( 'Import Demo', 'blockskit' ),
            'desc' => esc_html__( 'The quick demo import setup will help you configure your new website like theme demo.', 'blockskit' ),
            'button_label' => esc_html__( 'Import Demo', 'blockskit' ),
            'button_link' => esc_url( admin_url( 'themes.php?page=advanced-import' ) ),
            'is_button' => true,
            'is_new_tab' => false
        ),
        'four' => array(
            'title' => esc_html__( 'Documentation', 'blockskit' ),
            'desc' => esc_html__( 'We kindly request that you refer to the theme documentation for additional guidance on how to utilize its features.', 'blockskit' ),
            'button_label' => esc_html__( 'View Documentation', 'blockskit' ),
            'button_link' => esc_url( 'https://blockskit.com/docs' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'five' => array(
            'title' => esc_html__( 'Love Our Product?', 'blockskit' ),
            'desc' => esc_html__( 'We would greatly appreciate your motivation through a 5-star rating on our profile.', 'blockskit' ),
            'button_label' => esc_html__( 'Rate Us', 'blockskit' ),
            'button_link' => esc_url( 'https://wordpress.org/support/theme/' .$theme_name_config_url. '/reviews' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'six' => array(
            'title' => esc_html__( 'Have Question? Ask Us, Now!', 'blockskit' ),
            'desc' => esc_html__( 'We are committed to resolving any issues that may arise to ensure a seamless experience while using our theme.', 'blockskit' ),
            'button_label' => esc_html__( 'Get Support', 'blockskit' ),
    ...
    'free_pro' => array(
        array(
            'desc'=> __( 'Access to All Starter Sites', 'blockskit' ),
            'free' => __( 'no','blockskit' ),
            'pro'  => __( 'yes','blockskit' ),
        ), 
        array(
            'desc'=> __( 'Access to All Templates & Patterns', 'blockskit' ),
            'free' => __( 'no','blockskit' ),
            'pro'  => __( 'yes','blockskit' ),
        ),
        array(
            'desc'=> __( 'Work with the Page Builders', 'blockskit' ),
            'free' => __( 'no','blockskit' ),
            'pro'  => __( 'yes','blockskit' ),
        ),
        array(
            'desc'=> __( 'Creative & Unique Blocks', 'blockskit' ),
            'free' => __( 'no','blockskit' ),
            'pro'  => __( 'yes','blockskit' ),
        ),
        array(
            'desc'=> __( 'Elementor Ready', 'blockskit' ),
            'free' => __( 'no','blockskit' ),
            'pro'  => __( 'yes','blockskit' ),
        ),
        array(
            'desc'=> __( 'Priority Updates', 'blockskit' ),
            'free' => __( 'no','blockskit' ),
            'pro'  => __( 'yes','blockskit' ),
        ),
        array(
            'desc'=> __( 'Priority Support', 'blockskit' ),
            'free' => __( 'no','blockskit' ),
            'pro'  => __( 'yes','blockskit' ),
        ),
        array(
            'desc'=> __( 'Full Site Editing', 'blockskit' ),
            'free' => __( 'yes','blockskit' ),
            'pro'  => __( 'yes','blockskit' ),
        ),
        array(
            'desc'=> __( 'Customize Everything', 'blockskit' ),
            'free' => __( 'yes','blockskit' ),
            'pro'  => __( 'yes','blockskit' ),
        ),
        array(
            'desc'=> __( 'Lighting Fast', 'blockskit' ),
            'free' => __( 'yes','blockskit' ),
            'pro'  => __( 'yes','blockskit' ),
        ),
        array(
            'desc'=> __( 'One-click Demo Installer', 'blockskit' ),
            'free' => __( 'yes','blockskit' ),
            'pro'  => __( 'yes','blockskit' ),
        ),
        array(
            'desc'=> __( 'Fully Responsive', 'blockskit' ),
            'free' => __( 'yes','blockskit' ),
            'pro'  => __( 'yes','blockskit' ),
        ),
        array(
            'desc'=> __( 'Clean & Optimized Code', 'blockskit' ),
            'free' => __( 'yes','blockskit' ),
            'pro'  => __( 'yes','blockskit' ),
        ),
        array(
            'desc'=> __( 'Multiple Site Styles', 'blockskit' ),
            'free' => __( 'yes','blockskit' ),
            'pro'  => __( 'yes','blockskit' ),
        ),
        array(
            'desc'=> __( 'Global Settings', 'blockskit' ),
            'free' => __( 'yes','blockskit' ),
            'pro'  => __( 'yes','blockskit' ),
        ),
        array(
            'desc'=> __( 'CSS Animation', 'blockskit' ),
            'free' => __( 'yes','blockskit' ),
            'pro'  => __( 'yes','blockskit' ),
        ),
        array(
            'desc'=> __( 'Gutenberg Ready', 'blockskit' ),
            'free' => __( 'yes','blockskit' ),
            'pro'  => __( 'yes','blockskit' ),
        ),
        array(
            'desc'=> __( 'eCommerce Ready', 'blockskit' ),
            'free' => __( 'yes','blockskit' ),
            'pro'  => __( 'yes','blockskit' ),
        ),
        array(
            'desc'=> __( 'SEO Optimized', 'blockskit' ),
            'free' => __( 'yes','blockskit' ),
            'pro'  => __( 'yes','blockskit' ),
        ),
        array(
            'desc'=> __( 'Multilingual Compatibility', 'blockskit' ),
            'free' => __( 'yes','blockskit' ),
            'pro'  => __( 'yes','blockskit' ),
        ),
        array(
            'desc'=> __( 'RTL/LTR Language Support', 'blockskit' ),
            'free' => __( 'yes','blockskit' ),
            'pro'  => __( 'yes','blockskit' ),
        ),
    ),
