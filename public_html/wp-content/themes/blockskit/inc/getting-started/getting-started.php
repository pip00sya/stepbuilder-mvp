    ...
    public function bk_theme_info_welcome_admin_notice() {
        if ( is_admin() && !get_user_meta( get_current_user_id(), 'gs_notice_dismissed' ) ){
            ...
            if ( is_plugin_active( 'blockskit/blockskit.php' ) && is_plugin_active( 'advanced-import/advanced-import.php' ) ) {
                echo ('<div class="fs-notice success"><p>'.esc_html__( 'Essential plugins are added successfully. You may dismiss this notice now.', 'blockskit' ).'</p></div>');
            }else{
            echo ( '<p><strong>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s.', 'blockskit' ), $this->theme_name ) . '</strong></p><p class="plugin-notice">'.esc_html__( 'By clicking "Get Started," you will install and activate Blockskit & Advanced Import plugins to prepare your demo import.', 'blockskit' ).'</p><p><a href="#" class="bk-install-plugins button button-primary">' . sprintf( esc_html__( 'Get started with %s','blockskit' ), $this->theme_name ) . '</a></p>' );
            }
