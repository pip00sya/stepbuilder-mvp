...
if(! class_exists('product_attributes') ) :
    ...
    class product_attributes extends Field_Base {
        ...
        */
        ...
        function ajax_save_attributes()
        {
            $args = acf_parse_args(
              ...
              // validate
            if(!acf_verify_nonce('fea_form') ) { wp_send_json_error(__('Authentication Error. Please try refreshing the page.', 'acf-frontend-form-element'));
            }            
            
              $field = fea_instance()->frontend->get_field($args['attributes']);
            ...
            $return = array( 
                ...
                'form_objects' => fea_encrypt(json_encode($objects)) 
            );

            if( ! empty( $args['variations'] ) ) {
                $variations_field = fea_instance()->frontend->get_field( $args['variations'] );
        ...
        *  prepare_field()
        *
        *  This filter is appied to the $field before it is disaplayed
        *
        *  @type      filter
        *  @since      3.6
        *  @date      23/01/13
        ...
        */
        ...
        {            
            ...
            if( ! empty( $field['exclude_custom_attributes'] ) ){
                $field['blocks'] = array();
            }else{ 
        ...
        */
        ...
        function render_dropdowns( $field ) {
            // button label
            if ($field['button_label'] === '') {
                $field['button_label'] = __('Add Attribute', 'acf-frontend-form-element');
            }
            if ($field['save_text'] === '') {
                ...
                <div class="values">
                    ...
                    if (!empty($blocks)) :                        
                        ...
                        foreach( $field['value'] as $i => $value ):
                          ...
                          // render
                          $this->render_row_block($field, $blocks[ $value['woo_attrs'] ], $i, $value);
        ...
        */
        ...
        function render_field( $field )
        {
            ...
            if( 'dropdowns' == $field['display_format'] ){
                $this->render_dropdowns( $field );
                return;
            }
...
<div <?php acf_esc_attr_e($div); ?>>
      ...
      <script type="text-html" class="tmpl-popup"><?php 
        ...
        function get_attribute_fields( $block, $field )
        {
              ...
              $hide_visibilty = ! empty( $field['hide_visible_field'] );

              
              if( 'custom_attributes' == $block ) {
                  $sub_fields = array(
                    array_merge(
                        array(
                    ...
                    array_merge(
                        array(
                            ...
                            'wrapper' => [
                                  'width' => '',
                                  'class' => $hide_visibilty ? 'acf-hidden' : '',
                                  'id' => '',
                            ],
                  ...
                  $taxonomy = explode('block_', $block)[1] ?? false;

                  if( 'dropdowns' == $field['display_format'] ){
                    $taxonomy_label = wc_attribute_label('pa_' . $taxonomy);
                    $settings['terms']['label'] = $taxonomy_label;
                }
                    ...
                    array_merge(
                        array(
                          ...
                          'wrapper' => array(
                               ...
                               'allow_null' => 0,
                               'add_term' => ! empty( $field['allow_add_terms'] ),
                               'save_terms' => 0,
                               'load_terms' => 0,
...
<div <?php echo acf_esc_attr($div); ?>>
      ...
      <div class="acf-frontend-blocks-block-controls">
            <a class="acf-icon -plus small light acf-js-tooltip" href="#" data-name="add-block" title="<?php esc_attr_e('Add attribute', 'acf-frontend-form-element'); ?>"></a>
              <?php if('custom_product_attributes' == $block['name'] ) :?>
      ...
      <?php endif; ?>
        ...
        function render_row_block($field, $block, $i, $value)
        {
            // vars
            $order = 0;
            $el = 'div';
            ...
            <div <?php echo acf_esc_attr($div); ?>>
                ...
                <?php if (!empty($sub_fields)) : ?>
                                    ...
                                    <div class="acf-fields <?php if ($block['display'] == 'row') : ?>-left<?php endif; ?>">
        ...
        */
        ...
        function render_field_settings( $field )
        {
            acf_render_field_setting(
                $field, array(
                'label' => __('Display Format', 'acf-frontend-form-element'),
                'instructions' => __('Choose how to display the attributes. To use with Variations, you must use Table format', 'acf-frontend-form-element'),
                'type' => 'select',
                'name' => 'display_format',
                'choices' => array(
                    'dropdowns' => __('Dropdowns', 'acf-frontend-form-element'),
            ...
            acf_render_field_setting(
                ...
                'instructions' => __('Check this box to exclude the Custom Product Attributes option.', 'acf-frontend-form-element'),
                'type' => 'true_false',
                'name' => 'exclude_custom_attributes',
                'ui' => 1,
                )
            );
            ...
            acf_render_field_setting(
                ...
                'instructions' => __('Check this box to allow adding terms.', 'acf-frontend-form-element'),
                'type' => 'true_false',
                'name' => 'allow_add_terms',
                'ui' => 1,
                )
            );
            ...
            acf_render_field_setting(
                $field, array(
                'label' => __('Visible by Default', 'acf-frontend-form-element'),
                'instructions' => __('Check this box to make the attribute visible by default.', 'acf-frontend-form-element'),
                'type' => 'true_false',
            ...
            acf_render_field_setting(
                $field, array(
                    'label' => __('Hide Visiblity Field', 'acf-frontend-form-element'),
                    'instructions' => __('Check this box to hide the visible field.', 'acf-frontend-form-element'),
                    'type' => 'true_false',
                    'name' => 'hide_visible_field',
                    'ui' => 1,
                )
            );
        ...
        */
        ...
        {       
            ...
            if( 'dropdowns' == $field['display_format'] ){
                ...
                if ( $attribute_taxonomies) {
                    foreach ( $attribute_taxonomies as $tax) {
                        $rows[$i]['woo_attrs'] = $tax->attribute_name;
                        $current_attr = $product_atts['pa_' . $tax->attribute_name] ?? null;
                        $rows[$i][$tax->attribute_name . '_terms'] = isset( $current_attr) ? $current_attr->get_options() : [];
                        $rows[$i][$tax->attribute_name . '_locations'] = [];
                        if( isset( $current_attr) ){
                            if( $current_attr->get_visible() ) {
                                $rows[$i][$tax->attribute_name . '_locations'] = ['products_page'];
                            }
                        }else{
                            if( ! empty( $field['visible_by_default'] ) ){
                                $rows[$i][$tax->attribute_name . '_locations'] = ['products_page'];
                            }    
                        }

                        if (isset( $current_attr) && $current_attr->get_variation()) {
                            $rows[$i][$tax->attribute_name . '_locations'] = 'for_variations';
                        }
                        $i++;
                    }
                }
                ...
                if ( empty( $field['exclude_custom_attributes']) ) {
                    $current_attr = $product_atts['custom_product_attributes'] ?? null;
                    $rows[$i]['woo_attrs'] = 'custom_product_attributes';
                    $rows[$i]['name'] = isset( $current_attr) ? $current_attr->get_name() : 'Custom Attributes';
                    $rows[$i]['terms'] = isset( $current_attr) ? $current_attr->get_options() : [];

                    $rows[$i]['locations'] = [];
                    if( isset( $current_attr) ){
                     if( $current_attr->get_visible() ) {
                        $rows[$i]['locations'] = ['products_page'];
                     }
                    }else{
                        if( ! empty( $field['visible_by_default'] ) ){
                            $rows[$i]['locations'] = ['products_page'];
                    ...
                    if (isset( $current_attr) && $current_attr->get_variation()) {
                        $rows[$i]['locations'][] = 'for_variations';
                    }
                }
            ...
            if(get_post_type( $post_id) == 'product' ) {
                ...
                foreach( $product->get_attributes() as $attr_name => $attr ){
                    ...
                    $locations = array();
                    if( $attr['visible'] || ! empty( $field['visible_by_default'] ) ) {
