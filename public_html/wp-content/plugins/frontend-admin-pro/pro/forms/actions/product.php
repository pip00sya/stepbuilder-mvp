		...
		public function get_fields_display( $form_field, $local_field, $element = '', $sub_fields = false, $saving = false ) {
			...
			switch ( $form_field['field_type'] ) {
				...
				case 'attributes':
					...
					$local_field['exclude_custom_attributes'] = $form_field['exclude_custom_attributes'] ? 1 : 0;
					$local_field['allow_add_terms'] = $form_field['allow_add_terms'] ? 1 : 0;
					$local_field['no_value_msg']    = $form_field['no_value_msg'];
					$local_field['display_format']  = $form_field['display_format'] ?? 'table';
					if( ! empty( $sub_fields[0]['label'] ) ) {
